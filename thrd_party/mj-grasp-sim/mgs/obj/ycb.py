...
class ObjectYCB(CollisionMeshObject):
    ...
    @property
    def ycb_directory(self):
        return os.path.join(ASSET_PATH, "mj-objects/YCB")

    @property
    def asset_dir(self):
        return os.path.join(self.ycb_directory, self.object_id)

    @property
    def obj_file_path(self):
        return os.path.join(self.asset_dir, "textured.obj")

    @classmethod
    def all_object_ids(cls):
        all = []
        ycb_directory = os.path.join(ASSET_PATH, "mj-objects/YCB")
        ycb_obj_directories = os.listdir(ycb_directory)
        for x in ycb_obj_directories:
            all.append(x)
        return all
