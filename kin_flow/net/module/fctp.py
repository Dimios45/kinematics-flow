...
from flax.core.lift import id_fn
...
def _left_right(
    ...
    *,
    ...
    if sparse:
        ...
        einsum = einsum if custom_einsum_jvp else jnp.einsum
...
def _block_left_right(
    ...
    for ins in self.instructions:
        ...
        if ins.connection_mode == "uvw":
            ...
            out = jnp.einsum(
                "uvw,ijk,ui,vj->wk", w, w3j, x1, x2, precision=jax.lax.Precision.HIGHEST
            )
...
class FullyConnectedTP(TPWithWeightsAndBiases):
    def __init__(
        ...
        *,
        ...
        irreps_in1 = e3nn_jax.Irreps(in_irrep1)
        irreps_in2 = e3nn_jax.Irreps(in_irrep2)
        irreps_out = e3nn_jax.Irreps(out_irrep)
        instructions = [
            (i_1, i_2, i_out, connection_mode, True, 1.0)
